...
class LSModel_dense(torch.nn.Module):
    ...
    def __init__(self, corpus: Corpus_dense, network_framework: Dict):
        ...
        self.feature_size = 4 if "feature_size" not in network_framework else network_framework["feature_size"]

        # 保存
        self.save_path = "./ms_model/" if "save_path" not in network_framework else network_framework["save_path"]
        ...
        self.layer_dense_predict = torch.nn.Linear(in_features=self.hidden_size + self.feature_size,
    ...
    def forward(self, x, h, feature):
        ...
        out_nopad = torch.cat(
            [torch.cat(
                [out[i][:length_info[i], :], torch.Tensor([feature[i] for _ in range(length_info[i])]).to(self.device)],
                dim=1) for i in range(length_info.size()[0])], dim=0)
...
def train_stage(network_framework: Dict):
    ...
    # 构建网络
    lstmLM = LSModel_dense(corpus=corpus, network_framework=network_framework)
    ...
    for epoch in range(network_framework["num_epochs"]):
        ...
        for i in range(0, train_sample_count - batch_size + 1, batch_size):
            ...
            outputs, states = lstmLM(packed_inputs, states, input_tourist_feature)
        ...
        with torch.no_grad():
            ...
            for j in range(0, len(dev_corpus_lengths), batch_size):
                ...
                dev_packed_inputs = torch.nn.utils.rnn.pack_padded_sequence(input=dev_inputs.to(device),
                                                                            ...
                                                                            batch_first=True,
                                                                            enforce_sorted=False)
                dev_outputs, dev_states = lstmLM(dev_packed_inputs, dev_states, dev_input_tourist_feature)
...
def test_stage_batch(model_file: str, network_framework: Dict):
    ...
    def get_one_batch_sample_res(sentences_and_tragets: List):
        ...
        with torch.no_grad():
            ...
            test_packed_inputs = torch.nn.utils.rnn.pack_padded_sequence(input=test_input.to(device),
                                                                         ...
                                                                         batch_first=True,
                                                                         enforce_sorted=False)
            test_outputs, test_states = lstmLM(test_packed_inputs, test_states, test_feature)
        ...
        show_res = []
        for i in range(len(sample_to_test)):
            feature, sentence = sample_to_test[i]
            target = sample_target[i]
            target_index = predict_score_list[i][0].index(corpus.dictionary.word2index[target])
            show_res.append(
                [';'.join(sentence), target, target_index] + [corpus.dictionary.index2word[predict_score_list[i][0][j]]
                                                              for j in range(10)])

        return predict_score_list, show_res
    ...
    for j in range(0, len(test_pair_list), 256):
        ...
        predict_score_list_res, _show_res = get_one_batch_sample_res(test_pair_list[j:end_index])
    ...
    return {"MRR": sum(mrr_list) / val_cnt, "Top1": sum(topk_list[:1]) / val_cnt,
            "Top5": sum(topk_list[:5]) / val_cnt, "Top10": sum(topk_list[:10]) / val_cnt,
            "inf_cnt": inf_cnt, "val_cnt": val_cnt}, show_res


if __name__ == "__main__":
    test_model = './poi_lstm_dense_model_nobos_0423a/model_3/ms_model_lstm_1_bs_32_lr_2_embed_8_hidden_12_epoch_1477.mdl'
    framework = {'num_epochs': 1500, 'num_lstm_layers': 1, 'batch_size': 32, 'embed_size': 8, 'hidden_size': 12,
                 'learning_rate': 2e-05, 'report_frequency': 9999, 'best_model_count': 3, 'create_corpus': True,
                 'weight_decay': 0, 'feature_size': 4,
                 'corpus_path': './poi_lstm_dense_model_nobos_0423a/model_3/corpus.pkl',
                 'test_data_path': '../data/ms_test_feature_trips_pair.json',
                 'train_data_path': '../data/ms_train_feature_trips.json',
                 'save_path': './poi_lstm_dense_model_nobos_0423a/model_3',
                 'save_root_path': 'poi_lstm_dense_model_nobos_0423a',
                 'report_epochs': [5, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 200, 300, 400, 500, 600, 700,
                                   800, 900, 1000, 1050, 1100, 1150, 1200, 1250, 1300, 1350, 1400, 1450]}
    test_res, show_res = test_stage_batch(test_model, framework)
    # json.dump(show_res)
    with open("./poi_lstm_dense_model_nobos_0423a/poi_show_res.txt", 'w') as wf:
        for line in show_res:
