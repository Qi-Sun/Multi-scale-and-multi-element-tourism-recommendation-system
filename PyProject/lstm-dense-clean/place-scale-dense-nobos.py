        ...
        with torch.no_grad():
            ...
            if dev_loss_nbest[-1] == total_dev_loss:  # this means that a better model was founded
                ...
                cp_file_name = os.path.join(lstmLM.save_path,
                                            'place_scale_model_lstm_{}_bs_{}_lr_{}_embed_{}_hidden_{}_epoch_{}.mdl'.format(
                                                num_lstm_layers,
                                                batch_size,
...
if __name__ == "__main__":
    Network_Framework = {"num_epochs": 1500,
                         "num_lstm_layers": 1,
                         "batch_size": 32,
                         "embed_size": 16,
                         "hidden_size": 8,
                         ...
                         "save_root_path": "place_lstm_dense_model_nobos_0427a",
                         "report_epochs": []}
    ...
    model_path_id = 0
    res_list = []
    for lr in [2e-5, 1e-5]:  # [2e-5, 1e-5]
        for embed in [12]:  # [4, 8, 16]
            for hid in [4, 8, 16,24]:  # [4, 8, 16]
                for bs in [64, 32]:  # [64, 32]
                    for wd in [0,1e-10]:
                        ...
                        framework["report_epochs"] = list(range(5, 30, 5)) + list(range(30, 100, 10)) + list(
                            range(100, 1000, 100)) + list(range(1000, 1500, 50))
