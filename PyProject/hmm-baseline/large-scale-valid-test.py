...
if __name__ == "__main__":
    hmm_test_pair = json.load(open("../data/ls_test_feature_trips_pair.json", "r"))
    large_scale_trips = json.load(open("../data/ls_train_feature_trips.json", "r"))
    ...
    for n_iter in [25, 50, 75, 100, 300, 500, 750, 1000, 2000]:
        train_valid_info = ls_hmm_base.fit(n_iter=n_iter,
                                           save_file="./ls_model_0419/ls_hmm_comp_%s_iter_%s_len_%s.pkl" % (
                                               n_components, n_iter, 1 if use_lengths else 0), use_length=use_lengths)
        cur_res = ls_hmm_base.test_hmm_model(hmm_test_pair)
        cur_res["train_valid"] = train_valid_info
        cur_res["n_iter"] = n_iter
        res.append(cur_res)
    json.dump(res, open("./ls_model_0419/train_valid_test_res.json", 'w'))
    print(res)
