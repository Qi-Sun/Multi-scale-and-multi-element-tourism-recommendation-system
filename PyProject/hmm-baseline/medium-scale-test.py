...
if __name__ == "__main__":
    read_db_trip_into_file("./ms_model/medium_scale_trips_train.json", movement_field="ms_route_f",
                          train_or_test="train")
    read_db_trip_into_file("./ms_model/medium_scale_trips_test.json", movement_field="ms_route_f",
                          train_or_test="test")
    trip_test_list = json.load(open("./ms_model/medium_scale_trips_test.json", "r"))
    hmm_test_pair = get_hmm_test_pair(trip_test_list)
    pickle.dump(hmm_test_pair, open("./ms_model/medium_scale_trips_hmm_test_pair.plk", "wb"), 2)

    hmm_test_pair = pickle.load(open("./ms_model/medium_scale_trips_hmm_test_pair.plk", "rb"))
    medium_scale_trips = json.load(open("./ms_model/medium_scale_trips_train.json", "r"))
    n_components = 25
    n_iter = 1000
    use_lengths = True
    ms_hmm_base = medium_scale_hmm(medium_scale_trips=medium_scale_trips, n_components=n_components)
    ms_hmm_base.fit(n_iter=n_iter, save_file="./ms_model/ms_hmm_comp_%s_iter_%s_len_%s.pkl" % (
