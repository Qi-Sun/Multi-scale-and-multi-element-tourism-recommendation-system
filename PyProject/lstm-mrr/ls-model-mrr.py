    ...
    def read_corpus_from_list(self, sentences_list):
        ...
        def get_hmm_test_pair(trip_test_list):
            ...
            for trip in trip_test_list:
                ...
                hmm_test_pair.append(( ['<bos>'] + trip[:select_index], trip[select_index], trip))
            return hmm_test_pair
        
        self.dev_hmm_test_pair = get_hmm_test_pair(sentences_list[self.train_sample_count:])
...
def train_stage(network_framework: Dict):
    ...
    # 验证集的测试
    def get_dev_one_sample_res(sentence: List, target: str):
        ...
        for k in range(len(predict_score)):
            if predict_score[k][0] == corpus.dictionary.word2index[target]:
                return 1.0 / (k+1)
        return 0.0
        ...
        for i in range(0, train_sample_count - batch_size + 1, batch_size):
            ...
            if step % report_frequency == 0:
                ...
                with torch.no_grad():
                    total_dev_loss = 0
                    mrr_list = []
                    for dev_prefix, dev_res, _ in corpus.dev_hmm_test_pair:
                        predict_score = get_dev_one_sample_res(dev_prefix, dev_res)
                        if predict_score is not None:
                            mrr_list.append(predict_score)
                    total_dev_loss = sum(mrr_list) / len(mrr_list)
                    if len(dev_mrr_nbest) < num_best_models:
                        dev_mrr_nbest.append(total_dev_loss)
                    elif dev_mrr_nbest[-1] < total_dev_loss:
                        old_dev_loss = dev_mrr_nbest.pop(-1)
                        os.remove(checkpoint_file_dict[old_dev_loss])
                        dev_mrr_nbest.append(total_dev_loss)
                    else:
                        pass
                    if dev_mrr_nbest[-1] == total_dev_loss:  # this means that a better model was founded
                        # save model
                        cp_file_name = os.path.join(lstmLM.save_path,
                                                    'ls_model_embed_{}_hidden_{}_epoch_{}_batch_{}.mdl'.format(
                                                        embed_size,
                                                        hidden_size,
                                                        epoch + 1,
                                                        (step + 0)))
                        ...
                        print('Better Dev MRR: {:.4f}. Saved in {}'.format(total_dev_loss, cp_file_name))
                    dev_mrr_nbest.sort(reverse=True)

        print("Epoch {} Done".format(epoch))
    print("Start at : %s" % start_time)
    print("All Done! : %s" % datetime.datetime.now())
    print("Model Info:")
    for best_loss in dev_mrr_nbest:
        print("Filename : %s Loss : %.3f " % (checkpoint_file_dict[best_loss], best_loss))
    return [checkpoint_file_dict[best_loss] for best_loss in dev_mrr_nbest]
...
if __name__ == "__main__":
    Network_Framework = {"num_epochs": 20,
                         "num_lstm_layers": 1,
                         "batch_size": 64,
                         "embed_size": 32,
                         "hidden_size": 16,
    ...
    # test_stage("./ls_model/ls_model_embed_48_hidden_32_epoch_4_batch_40.mdl",Network_Framework)
