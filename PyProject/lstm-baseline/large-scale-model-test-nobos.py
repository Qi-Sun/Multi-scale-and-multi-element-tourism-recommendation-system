...
def train_stage(network_framework: Dict):
    ...
    if not os.path.exists(corpus_path) or network_framework["create_corpus"]:
        ...
        corpus.read_corpus_from_list(large_scale_trips)
        ...
        with torch.no_grad():
            ...
            if dev_loss_nbest[-1] == total_dev_loss:  # this means that a better model was founded
                ...
                cp_file_name = os.path.join(lstmLM.save_path,
                                            'ls_model_lstm_{}_bs_{}_lr_{}_embed_{}_hidden_{}_epoch_{}.mdl'.format(
                                                num_lstm_layers,
                                                batch_size,
...
def test_stage_batch(model_file: str, network_framework: Dict):
    ...
    # 测试单条数据
    def get_one_batch_sample_res_nobos(sentences_and_tragets: List):
        sample_to_test = []
        sample_target = []
        for sentence, target, _ in sentences_and_tragets:
            if any([x not in corpus.dictionary.word2index for x in sentence + [target]]):
                pass
            else:
                sample_to_test.append(sentence)
                sample_target.append(target)

        num_lstm_layers = network_framework["num_lstm_layers"]
        hidden_size = network_framework["hidden_size"]
        test_states = (torch.zeros(num_lstm_layers, len(sample_to_test), hidden_size),
                       torch.zeros(num_lstm_layers, len(sample_to_test), hidden_size))
        predict_score_list = []

        test_input_length, test_input = corpus.get_train_or_dev_packed_seq(sample_to_test, test_stage=True)
        with torch.no_grad():
            test_input_length = torch.LongTensor(test_input_length)
            test_packed_inputs = torch.nn.utils.rnn.pack_padded_sequence(input=test_input,
                                                                         lengths=test_input_length,
            ...
            for index, last_word_index in enumerate(torch.cumsum(test_input_length, dim=0)):
                ...
                predict_score_list.append(
                    ([x[0] for x in predict_score if x[0] > 3], corpus.dictionary.word2index[sample_target[index]]))
    ...
    for j in range(0, len(test_pair_list), 256):
        ...
        predict_score_list_res = get_one_batch_sample_res_nobos(test_pair_list[j:end_index])
...
if __name__ == "__main__":
    Network_Framework = {"num_epochs": 300,
                         "num_lstm_layers": 1,
                         "batch_size": 32,
                         "embed_size": 16,
                         "hidden_size": 8,
                         ...
                         "best_model_count": 3,
                         "create_corpus": True,
                         "corpus_path": "./ls_model/corpus.pkl",
                         "test_data_path": "./large_scale_trips_hmm_test_pair.plk",
                         "save_path": "./ls_grid_search_model/",
                         "report_epochs": []}
    ...
    model_path_id = 1
    res_list = []
    while os.path.exists("./ls_grid_search_model_0416a/model_%s" % model_path_id):
        model_save_path = "./ls_grid_search_model_0416a/model_%s" % model_path_id

        cur_model_res = []
        model_test_info = json.load(open(os.path.join(model_save_path, "model_and_test_info.json"), 'r'))
        model_info = {}
        for test_res, filename, cur_framework in model_test_info:
            model_info[filename] = cur_framework
        for filename in os.listdir(model_save_path):
            fullfilename = os.path.join(model_save_path, filename)
            if fullfilename not in model_info:
                continue
            test_res = test_stage_batch(fullfilename, model_info[fullfilename])
            cur_res = [test_res, os.path.join(fullfilename), model_info[fullfilename]]
            cur_model_res.append(cur_res)
            res_list.append(cur_res)
        json.dump(cur_model_res,
                  open(os.path.join(model_save_path, "model_and_test_nobos_info.json"), 'w'))

        print("Model % s End At %s" % (model_path_id, datetime.datetime.now()))
        model_path_id += 1

    pickle.dump(res_list, open("./ls_grid_search_model_0416a/grid_search_model_no_bos_res.pkl", 'wb'))
    json.dump(res_list, open("./ls_grid_search_model_0416a/grid_search_model_no_bos_res.json", 'w'))
