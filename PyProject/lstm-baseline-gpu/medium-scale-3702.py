...
def train_stage(network_framework: Dict):
    ...
    if not os.path.exists(corpus_path) or network_framework["create_corpus"]:
        corpus = Corpus()
        medium_scale_trips = json.load(open("./medium_scale_trips_train.json", 'r'))
...
if __name__ == "__main__":
    Network_Framework = {"num_epochs": 300,
                         ...
                         "test_data_path": "./medium_scale_trips_hmm_test_pair.plk",
            ...
            for hid in [8, 16, 24, 32]:
                for bs in [64, 32]:
                    ...
                    framework["report_epochs"] = [5, 10, 15, 25, 30, 35, 40, 50, 60, 70, 80, 90, 100, 125, 150, 200,
                                                  250]
                    framework["save_path"] = "./ms_grid_search_model_0416b/model_%s" % model_path_id
                    ...
                    sys.stdout.write("Model % s End At %s" % (model_path_id, datetime.datetime.now()))
    pickle.dump(res_list, open("./ms_grid_search_model_0416b/grid_search_model_res.pkl", 'wb'))
    json.dump(res_list, open("./ms_grid_search_model_0416b/grid_search_model_res.json", 'w'))
